...
space_metatable = function(remote)
    ...
    function methods:select(key, opts)
        ...
        return remote:_request('select', opts, self.id, 0, iterator,
                               offset, limit, key)
    end

    function methods:delete(key, opts)
        space_check(self, 'delete')
        return one_tuple(remote:_request('delete', opts, self.id, 0, key))
    end

    function methods:update(key, oplist, opts)
        space_check(self, 'update')
        return one_tuple(remote:_request('update', opts, self.id, 0, key,
                                         oplist))
    end

    function methods:upsert(key, oplist, opts)
        space_check(self, 'upsert')
        return one_tuple(remote:_request('upsert', opts, self.id, 0, key,
                                         oplist))
    end

    function methods:get(key, opts)
        space_check(self, 'get')
        local res = remote:_request('select', opts, self.id, 0, box.index.EQ,
                                    0, 2, key)
...
index_metatable = function(remote)
    ...
    function methods:upsert(key, oplist, opts)
        ...
        local res = remote:_request('upsert', opts, self.space.id, self.id,
                                    key, oplist)
        return one_tuple(res)
    end
